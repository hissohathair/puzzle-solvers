...
SOLVED_PUZZLE = [
	...
	[6, 3, 9, 5, 2, 1, 4, 7, 8],
	[4, 2, 7, 8, 9, 6, 5, 1, 3]]
LEGAL_MOVES = [[0, 2, 3], [3, 3, 1], [4, 4, 6], 
			[1, 6, 7], [7, 3, 5], [1, 8, 2],
			[8, 2, 7], [8, 6, 5], [5, 0, 3], 
			[5, 7, 8], [6, 4, 3]]
ILLEGAL_MOVES = [[0, 2, 8], [0, 2, 1], [3, 3, 2], 
				[3, 3, 4], [2, 2, 4], [3, 3, 6],
				[0, 0, 9], [0, 0, 1], [2, 2, 4], 
				[2, 3, 4]]
...
class TestSudoku(unittest.TestCase):
	...
	def test_box_num_toxy(self):
		...
		for i in range(self.p.max_value()):
			with self.subTest(f"Cage {i} at {correct_values[i]}"):
				...
				self.assertEqual(self.p.box_xy_to_num(correct_values[i][0], correct_values[i][1]), i)
		return
		...
		for x in range(self.p.max_value()):
			for y in range(self.p.max_value()):
				with self.subTest(f"Cage for {x},{y}"):
					...
					self.assertEqual(self.p.box_xy_to_num(x,y), self.p.box_xy_to_num(pos[0],pos[1]))
		return
	...
	def test_empty_cells(self):
		...
		with self.subTest(f"Basic empty cell count check"):
			...
			self.assertEqual(len(self.p.get_all_empty_cells()), self.p.num_empty_cells())
			self.assertEqual(len(self.s.get_all_empty_cells()), 0)
			self.assertEqual(len(self.s.get_all_empty_cells()), self.s.num_empty_cells())
